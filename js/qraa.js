...
var defautQrimgsize;
	function qraa(myurl) { //网站LOGO正方形图片相对路径
		if (document.getElementById("showqr")) {
		} else {

			$('body').append('<div id="showqr" style="z-index:99999;position: fixed;' + LiftOrRight + myqrtop +'"><p id="myqrnote" align="center">' + myqrnote + '</p><img  id="logoimg" src="' + myqrlogimgsrc + '" style="display:none"/></div>');
			//$('#somenews').html('<iframe height=100% width=100% src="http://www.gbtags.com/gb/rtwidget-replayerpreview/680.htm" frameborder=0 allowfullscreen></iframe>')
	...
	var logimgheight=$("#logoimg").height()
		...
		if (logimgheight&&myqrlogimgsrc&&myqrlogimgsrc.indexOf(thiswwwurl) >= 0) //避免图片跨域
		{
...
var X,Y,pagex,pagey;
$("#showqr").on('touchstart',function(e) {
	  Y= $('#showqr').offset().top; 
	 X = $('#showqr').offset().left; 
	 pagex=e.originalEvent.targetTouches[0].pageX;
	 pagey=e.originalEvent.targetTouches[0].pageY;
	//	$("#myqrnote").html(pagex+pagey)
});

$("#showqr").on('touchend',function(e) {
	 pagex=e.originalEvent.changedTouches[0].pageX-pagex+X;
	 pagey=e.originalEvent.changedTouches[0].pageY-pagey+Y;
	 if (pagex<0)
	 {
		 pagex=0;
	 };
	...
	pagex=pagex+"px";
	pagey=pagey+"px";
$("#showqr").css("position","absolute");
	$("#showqr").css("left",pagex);
$("#showqr").css("top",pagey);
	 //alert(pagex)
});

    //  $(".g-bd").unbind('swipeleft').swipeleft(function(){
		$("#showqr img").unbind('click').click(function(){
			var thiswidth = $(this).width()
			if (thiswidth == myqrsize || thiswidth > 300) {
				var smallqrsize = Number(myqrsize) / 6
				$("#qrAnonymous").css({
					"height": smallqrsize,
					"width": smallqrsize
				})
...
var myDate = new Date();
var timenow=myDate.getTime();
 var i=6;//播放时间
if (timenow-time>1000)
{
$("#somenews").html("<div>点击二维码放大</div><div id='showtime'></div>");//广告
$("#somenews").show();
 setTimeout(function () {$("#somenews").hide();}, i*1000);
 function addtime(){
 setTimeout(function () {$("#showtime").text(i--);addtime();}, 1000);
 }
 i=i-1;
 addtime()
	 if (i<0)
	...
	setTimeout(function () {updata(1472735708257);}, 3000);//延时弹出
*/	
